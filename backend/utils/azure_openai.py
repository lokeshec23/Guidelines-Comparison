...
#         try:
#             chunk_result = json.loads(cleaned_content)
#         except json.JSONDecodeError:
#             print(f"✗ JSON parse error in chunk {idx+1}, skipping. Content preview:\n{repr(raw_content[:500])}")
#             continue
#         # Merge results
...
# def generate_from_chunks(chunks: List[str], prompt_template: str) -> str:
#     if len(chunks) == 1:
...
# {text}
# """
#     return generate_from_chunks(chunks, template)
...
# ### OUTPUT FORMAT (JSON ONLY)
# {{
#   "Major Section Title": {{"summary": "Short description of what this section covers.","Subsection Title": "Condensed key rules, eligibility, or conditions under that subsection."}},
#   "Next Major Section": {{"summary": "...","Another Subsection": "..."}}
# }}
...
# -----------------------
def generate_structured_output(chunks: List[str], task: str) -> str:
    """
    Generic function to generate JSON output for any type of task.
    task: "taxonomy" | "ontology" | "semantics" | "rules"
    """
    prompt_templates = {
        "taxonomy": """
You are an expert mortgage guideline analyst specializing in document taxonomy extraction.
...
### INSTRUCTIONS
1. Identify major sections and subsections based on titles, numbering, or formatting
2. For each **major section**: Add a "summary" (2-3 lines)
3. For each **subsection**: Provide rules/conditions only
4. Keep headings exactly as written in the text
5. Maintain hierarchy and section order
6. Do NOT hallucinate beyond the text

### OUTPUT FORMAT (JSON ONLY)
{{
  "Major Section Title": {{
    "summary": "Short description",
    "Subsection Title": "Key rules/conditions"
  }}
}}
...
"""
    ...
    if task not in prompt_templates:
        raise ValueError(f"Unknown task '{task}'. Must be one of {list(prompt_templates.keys())}")
...
# -----------------------
def generate_taxonomy(chunks: List[str]) -> str:
    return generate_structured_output(chunks, "taxonomy")


def generate_ontology(chunks: List[str]) -> str:
    return generate_structured_output(chunks, "ontology")


def generate_semantics(chunks: List[str]) -> str:
    return generate_structured_output(chunks, "semantics")


def generate_rules(chunks: List[str]) -> str:
    return generate_structured_output(chunks, "rules")
